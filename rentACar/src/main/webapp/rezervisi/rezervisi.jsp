				...
				<td class="right">
					<select name="vozniPark">
						...
						<c:forEach var="vp" items="${autalista}">
								<option value="${vp.registracija }">${vp.marka} ${vp.model}, Boja: ${vp.boja}</option>
						</c:forEach>
					</select>
				</td>
			</tr>
				...
				<td class="right">
					<select name="dodaci"> 
						<c:forEach var="dod" items="${dodacilista}">
								<option value="${dod.idDodatka }">${dod.naziv } ${dod.cena }e</option>
							</c:forEach> 
 					</select>
				</td>
			</tr>
			<tr><td colspan="2"><button class="btn btn-success active btn-md pull-right" id="potvrdi"type="submit">Iznajmi</button></td></tr>
			<tr><td colspan="2"><center>${poruka }</center></td></tr>
		</table>
		</form>
