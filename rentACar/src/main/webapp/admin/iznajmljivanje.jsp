...
<center><button class="btn btn-success active " onclick="PrintElem()">Stampaj slip</button>
				<button class="btn btn-success active " onclick="email()">Posalji email klijentu</button>
				</center>
		<p hidden id="email" name="email">${email }</p>
	</form>
<script>function email(){window.open('mailto:'+document.getElementById("email").innerHTML+'?subject=RentACar Novi Sad');}</script>
